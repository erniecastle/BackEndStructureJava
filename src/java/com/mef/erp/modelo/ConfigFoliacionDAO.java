...
public class ConfigFoliacionDAO extends GenericHibernateDAO<ConfigFoliacion, Long>
        implements ConfigFoliacionDAOIF {
    ...
    public Mensaje agregar(ConfigFoliacion entity, String uuidCxn) {
        ConfigFoliacion categoriasPuestos;
        try {
    ...
    public Mensaje getConfigFoliacionAll(String uuidCxn) {
        List<ConfigFoliacion> values;
        try {
            inicializaVariableMensaje();
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            q = getSession().createQuery("from ConfigFoliacion order by tabla,campoClave");
            values = q.list();
            ...
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("getConfigFoliacionAll()1_Error: ").append(ex));
            try {
                if (getSession().getTransaction().isActive()) {
                    getSession().getTransaction().rollback();
                }
    ...
    public Mensaje getConfigFoliacionPorClave(String tabla, String campoClave, String uuidCxn) {
        ...
        try {
            ...
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            q = getSession().createQuery("from ConfigFoliacion where tabla=:tabla and campoClave=:clave");
            q.setString("clave", campoClave);
            q.setString("tabla", tabla);
            n = (ConfigFoliacion) q.uniqueResult();
            mensajeResultado.setResultado(n);
            mensajeResultado.setNoError(0);
            mensajeResultado.setError("");
            getSession().getTransaction().commit();
        } catch (HibernateException ex) {
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("getConfigFoliacionPorClave()1_Error: ").append(ex));
            try {
                if (getSession().getTransaction().isActive()) {
                    getSession().getTransaction().rollback();
                }
    ...
    public Mensaje consultaPorRangos(Integer inicio, Integer rango, String uuidCxn) {
        List<ConfigFoliacion> listCategoriasPuesto;
        try {
            inicializaVariableMensaje();
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            listCategoriasPuesto = (List<ConfigFoliacion>) consultaPorRangos(inicio, rango, null, null);
    ...
    public Mensaje saveDeleteConfigFoliacion(List<ConfigFoliacion> entitysCambios, Object[] idDelete, int rango, String uuidCxn) {
        List<ConfigFoliacion> listCategoriasPuesto = new ArrayList<ConfigFoliacion>();
        try {
            inicializaVariableMensaje();
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            ...
            if (commit) {
                if (!entitysCambios.isEmpty()) {
                    ...
                    for (i = 0; i < entitysCambios.size(); i++) {
                        listCategoriasPuesto.add(makePersistent(entitysCambios.get(i)));
                        if (i % rango == 0 & i > 0) {
                            flush();
                            clear();
                        }
            ...
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("saveDeleteConfigFoliacion()1_Error: ").append(ex));
            try {
                if (getSession().getTransaction().isActive()) {
                    getSession().getTransaction().rollback();
                }
