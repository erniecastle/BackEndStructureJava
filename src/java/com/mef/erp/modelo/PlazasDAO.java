/**
 * @author: daniel Fecha de Creación: --/--/---- Compañía:Finesoftware
 * Descripción del programa:
 * -----------------------------------------------------------------------------
 * MODIFICACIONES:
 * -----------------------------------------------------------------------------
 * Clave: JSA01 Autor: Jose Armando Sanchez Acosta Fecha: 12-04-2012
 * Descripción: Se agrego la relacion con la razonSocial y se le agrego el
 * filtro a los metodos. Tambien se elimino el metodo getPlazasAll ya que las
 * plazas son por razon social.
 * -----------------------------------------------------------------------------
 */
 ...
 * @author
 */
public class PlazasDAO extends GenericHibernateDAO<Plazas, Long>
        implements PlazasDAOIF {
    ...
    private List<Plazas> listEsp = new ArrayList<Plazas>();
    ...
    public Mensaje eliminar(Plazas entity, String uuidCxn) {
        try {
            ...
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            deleteListQuery("Plazas", "id", new Object[]{entity.getId()});
            clear();
            mensajeResultado.setResultado(true);
            mensajeResultado.setNoError(0);
            mensajeResultado.setError("");
    ...
    public Mensaje getPlazasPorClave(String clave, String razonSocial, String uuidCxn) {//JSA01
        Plazas r;
        try {
            inicializaVariableMensaje();
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            q = getSession().createQuery("from Plazas p where p.clave=:clave and p.razonesSociales.clave=:claveRazonSocial");
            q.setString("clave", clave);
            q.setString("claveRazonSocial", razonSocial);
            r = (Plazas) q.uniqueResult();
            mensajeResultado.setResultado(r);
            mensajeResultado.setNoError(0);
            mensajeResultado.setError("");
            getSession().getTransaction().commit();
        } catch (HibernateException ex) {
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("getplazasPorClave()1_Error: ").append(ex));
            try {
                if (getSession().getTransaction().isActive()) {
                    getSession().getTransaction().rollback();
                }
    ...
    public Mensaje getPlazasPorRazonSocial(String clave, String uuidCxn) {//JSA01
        List<Plazas> values;
        try {
            inicializaVariableMensaje();
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            q = getSession().createQuery("from Plazas p where p.razonesSociales.clave=:clave");
            q.setString("clave", clave);
            values = q.list();
            ...
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("getPlazasPorRazonSocial()1_Error: ").append(ex));
            try {
                if (getSession().getTransaction().isActive()) {
                    getSession().getTransaction().rollback();
                }
    ...
    public Mensaje consultaPorRangos(Integer inicio, Integer rango, String uuidCxn) {
        try {
            ...
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            listEsp = (List<Plazas>) consultaPorRangos(inicio, rango, null, null);
    ...
    public Mensaje consultaPorFiltrosPlazas(String query, Object[] campos, Object[] valores, String uuidCxn) {
        try {
            inicializaVariableMensaje();
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            listEsp = (List<Plazas>) consultaPorFiltros(query, campos, valores, null, null);
            mensajeResultado.setResultado(listEsp);
            mensajeResultado.setNoError(0);
            mensajeResultado.setError("");
            getSession().getTransaction().commit();
    ...
    public Mensaje existeDato(String campo, Object valor, String uuidCxn) {
        ...
        try {
            ...
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            existe = existeDato("Plazas", campo, valor);
