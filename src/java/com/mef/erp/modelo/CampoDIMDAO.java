 ...
 * CampoDIMDAO para llamados a metodos de HIBERNATE
 * -----------------------------------------------------------------------------
 * MODIFICACIONES:
 * -----------------------------------------------------------------------------
 ...
 * @author daniel
 */
public class CampoDIMDAO extends GenericHibernateDAO<CampoDIM, Long>
        implements CampoDIMDAOIF {
    ...
    private List<CampoDIM> listEsp = new ArrayList<CampoDIM>();
    ...
    public Mensaje agregar(CampoDIM entity, String uuidCxn) {//JSA01
        ...
        try {
            ...
            campoDIM = makePersistent(entity);
            mensajeResultado.setResultado(campoDIM);
    ...
    public Mensaje getCampoDIMAll(String uuidCxn) {//JSA01
        List<CampoDIM> values;
        try {
            inicializaVariableMensaje();
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            q = getSession().createQuery("from CampoDIM");
            values = q.list();
            ...
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("getCampoDIMAll()1_Error: ").append(ex));
            try {
                if (getSession().getTransaction().isActive()) {
                    getSession().getTransaction().rollback();
                }
    ...
    public Mensaje getCampoDIMPorClave(String clave, String uuidCxn) {//JSA01
        ...
        try {
            ...
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            q = getSession().createQuery("from CampoDIM where clave=:clave");
            q.setParameter("clave", clave);
            r = (CampoDIM) q.uniqueResult();
            mensajeResultado.setResultado(r);
            mensajeResultado.setNoError(0);
            mensajeResultado.setError("");
            getSession().getTransaction().commit();
        } catch (HibernateException ex) {
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("getCampoDIMPorClave()1_Error: ").append(ex));
            try {
                if (getSession().getTransaction().isActive()) {
                    getSession().getTransaction().rollback();
                }
    ...
    public Mensaje consultaPorRangos(Integer inicio, Integer rango, String uuidCxn) {//JSA01
        try {
            ...
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            listEsp = (List<CampoDIM>) consultaPorRangos(inicio, rango, null, null);
    ...
    public Mensaje existeDato(String campo, Object valor, String uuidCxn) {//JSA01
        ...
        try {
            ...
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            existe = existeDato("CampoDIM", campo, valor);
