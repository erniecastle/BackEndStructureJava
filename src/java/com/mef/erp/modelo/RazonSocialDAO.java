...
public class RazonSocialDAO extends GenericHibernateDAO<RazonSocial, String>
        implements RazonSocialDAOIF {
    ...
    public Mensaje agregar(RazonSocial entity, String uuidCxnMaestra) {
        ...
        try {
            ...
            razonSocial = makePersistent(entity);
            mensajeResultado.setResultado(razonSocial);
    ...
    public Mensaje getRazonSocialAll(String uuidCxnMaestra) {//JSA01
        List<RazonSocial> values;
        try {
            inicializaVariableMensaje();
            setSession(HibernateUtil.currentSession(uuidCxnMaestra));
            getSession().beginTransaction();
            ...
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("getRazonSocialAll()1_Error: ").append(ex));
            try {
                if (getSession().getTransaction().isActive()) {
                    getSession().getTransaction().rollback();
                }
    ...
    public Mensaje getRazonPorClave(String claves, String uuidCxnMaestra) {
        RazonSocial listRazonSocial;
        try {
            inicializaVariableMensaje();
            setSession(HibernateUtil.currentSession(uuidCxnMaestra));
            getSession().beginTransaction();
            q = getSession().createQuery("from RazonSocial r where r.claveRazonSocial =:clave ");
            q.setParameter("clave", claves);
            listRazonSocial = (RazonSocial) q.uniqueResult();
            mensajeResultado.setResultado(listRazonSocial);
            mensajeResultado.setNoError(0);
            mensajeResultado.setError("");
            getSession().getTransaction().commit();
        } catch (HibernateException ex) {
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("getRazonPorClave()1_Error: ").append(ex));
            try {
                if (getSession().getTransaction().isActive()) {
                    getSession().getTransaction().rollback();
                }
