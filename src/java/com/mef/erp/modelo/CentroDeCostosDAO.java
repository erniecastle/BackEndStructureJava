 ...
 * Descripción del programa: inteface centro de costos para llamados a metodos
 * de HIBERNATE
 * -----------------------------------------------------------------------------
 * MODIFICACIONES:
 * -----------------------------------------------------------------------------
...
public class CentroDeCostosDAO extends GenericHibernateDAO<CentroDeCosto, String>
        implements CentroDeCostosIF {
    ...
    private List<CentroDeCosto> listEsp = new ArrayList<CentroDeCosto>();
    ...
    public Mensaje agregar(CentroDeCosto entity, String uuidCxn) {//JSA01
        ...
        try {
            ...
            centroDeCosto = makePersistent(entity);
            mensajeResultado.setResultado(centroDeCosto);
    ...
    public Mensaje getCentroDeCostoAll(String claveRazonesSocial, String uuidCxn) {//JSA01
        List<CentroDeCosto> values;
        try {
            inicializaVariableMensaje();
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            q = getSession().createQuery("from CentroDeCosto c where c.razonesSociales.clave=:clave");
            ...
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("getCentroDeCostoAll()1_Error: ").append(ex));
            try {
                if (getSession().getTransaction().isActive()) {
                    getSession().getTransaction().rollback();
                }
    ...
    public Mensaje getCentroDeCostoPorClave(String clave, String claveRazonesSocial, String uuidCxn) {//JSA01
        ...
        try {
            ...
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            q = getSession().createQuery("from CentroDeCosto c where c.clave=:clave and c.razonesSociales.clave=:claveRazonSocial");
            q.setParameter("clave", clave);
            q.setParameter("claveRazonSocial", claveRazonesSocial);
            r = (CentroDeCosto) q.uniqueResult();
            mensajeResultado.setResultado(r);
            mensajeResultado.setNoError(0);
            mensajeResultado.setError("");
            getSession().getTransaction().commit();
        } catch (HibernateException ex) {
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("getCentroDeCostoPorClave()1_Error: ").append(ex));
            try {
                if (getSession().getTransaction().isActive()) {
                    getSession().getTransaction().rollback();
                }
    ...
    public Mensaje getCentroDeCostoPorClaveYRazon(String clave, String claveRazon, String uuidCxn) {//JSA01
        ...
        try {
            ...
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            q = getSession().createQuery("from CentroDeCosto c where c.clave=:clave and c.razonesSociales.clave=:claveRazon");
            q.setParameter("clave", clave);
            ...
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("getCentroDeCostoPorClaveYRazon()1_Error: ").append(ex));
            try {
                if (getSession().getTransaction().isActive()) {
                    getSession().getTransaction().rollback();
                }
    ...
    public Mensaje consultaPorRangosCentroCosto(Integer inicio, Integer rango, String[] camposWhere, Object[] camposWhereValores, String uuidCxn) {
        try {
            inicializaVariableMensaje();
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            listEsp = (List<CentroDeCosto>) consultaPorRangos(inicio, rango, camposWhere, camposWhereValores);
    ...
    public Mensaje consultaPorFiltrosCentroCosto(String query, Object[] campos, Object[] valores, Integer inicio, Integer rango, String uuidCxn) {//JSA01
        try {
            inicializaVariableMensaje();
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            listEsp = (List<CentroDeCosto>) consultaPorFiltros(query, campos, valores, inicio, rango);
            mensajeResultado.setResultado(listEsp);
            mensajeResultado.setNoError(0);
            mensajeResultado.setError("");
            getSession().getTransaction().commit();
    ...
    public Mensaje saveDeleteCentroDeCosto(List<CentroDeCosto> entitysCambios, Object[] clavesDelete, int rango, String uuidCxn) {//JSA01
        listEsp = new ArrayList<CentroDeCosto>();
        try {
            commit = true;
            inicializaVariableMensaje();
            setSession(HibernateUtil.currentSession(uuidCxn));
            ...
            entitysCambios = (entitysCambios == null ? new ArrayList<CentroDeCosto>() : entitysCambios);
            if (commit && !entitysCambios.isEmpty()) {
                listEsp = agregarListaCentrosDeCostos(entitysCambios, rango);
            }
            if (commit) {
                mensajeResultado.setResultado(listEsp);
                mensajeResultado.setNoError(0);
            ...
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("saveDeleteCentroDeCosto()1_Error: ").append(ex));
            try {
                if (getSession().getTransaction().isActive()) {
                    getSession().getTransaction().rollback();
                }
    ...
    private List<CentroDeCosto> agregarListaCentrosDeCostos(List<CentroDeCosto> entitys, int rango) {//JSA01
        if (listEsp != null) {
            listEsp.clear();
        }
        try {
    ...
    private void deleteListQuerys(String tabla, String campo, Object[] valores) {//JSA01
        try {
