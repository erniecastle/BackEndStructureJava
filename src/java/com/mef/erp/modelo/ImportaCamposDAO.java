/**
 * @author: Jose Ernesto Valenzuela Castillo Fecha de Creación: 24/01/2014
...
import com.mef.erp.modelo.entidad.DetalleImportaCampos;
 ...
 * @author Ernesto
 */
public class ImportaCamposDAO extends GenericHibernateDAO<ImportaCampos, String>
        implements ImportaCamposDAOIF {
    ...
    public Mensaje agregar(ImportaCampos entity, String uuidCxn) {
        ...
        try {
            ...
            importaCampos = makePersistent(entity);
            mensajeResultado.setResultado(importaCampos);
            mensajeResultado.setNoError(0);
            mensajeResultado.setError("");
            getSession().getTransaction().commit();
        } catch (HibernateException ex) {
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("agregar()_Error: ").append(ex));
            try {
    ...
    public Mensaje getImportaCamposAll(String uuidCxn) {
        List<ImportaCampos> values;
        try {
            inicializaVariableMensaje();
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            query = getSession().createQuery("from ImportaCampos");
            values = query.list();
            ...
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("getImportaCamposAll()1_Error: ").append(ex));
            try {
                if (getSession().getTransaction().isActive()) {
                    getSession().getTransaction().rollback();
                }
    ...
    public Mensaje getImportaCamposPorVentana(String nombreVentana, String uuidCxn) {
        List<ImportaCampos> values = null;
        try {
            inicializaVariableMensaje();
            setSession(HibernateUtil.currentSession(uuidCxn));
            getSession().beginTransaction();
            query = getSession().createQuery("from ImportaCampos o where o.nombreVentana = :nombreVentana");
            query.setParameter("nombreVentana", nombreVentana);
            values = query.list();
            mensajeResultado.setResultado(values);
            mensajeResultado.setNoError(0);
            mensajeResultado.setError("");
            ...
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("getImportaCamposPorVentana()1_Error: ").append(ex));
            try {
                if (getSession().getTransaction().isActive()) {
                    getSession().getTransaction().rollback();
                }
    ...
    public Mensaje saveDeleteImportaCampos(ImportaCampos entity, Object[] eliminarDetalleImportCampos, String uuidCxn) {
        ...
        try {
            ...
            if (eliminarDetalleImportCampos != null && eliminarDetalleImportCampos.length > 0) {
                commit = deleteListQuerys(DetalleImportaCampos.class.getCanonicalName(), "Id", eliminarDetalleImportCampos);
            }
            if (commit) {
                i = entity;
            ...
            getSession().getTransaction().rollback();
            System.err.println(concatena.delete(0, concatena.length()).append(msgError).append("saveDeleteImportaCampos()1_Error: ").append(ex));
            try {
                if (getSession().getTransaction().isActive()) {
                    getSession().getTransaction().rollback();
                }
